...
#'  "year", or "total".
#'
#' @importFrom dplyr across all_of group_by mutate rename starts_with summarize
#' @importFrom stringr str_replace
#' @importFrom tidyr pivot_longer
#' @importFrom rlang :=
summarize_agg_data <- function(data, level) {
  group_level <- switch(level,
    "month" = "day",
    "year" = "month",
    "total" = "year"
  )
  ...
  data %>%
    ...
    summarize(across(starts_with("kwh_"), \(x) mean(x, na.rm = TRUE))) %>%
    pivot_longer(
      -all_of(group_level),
      names_to = "wattage_type", values_to = "kwh"
    ) %>%
    rename(date := group_level) %>%
    mutate(wattage_type = str_replace(wattage_type, "^kwh_", ""))
}
